...
function mapUserForMessaging(user) {
    return {
        id: user.userId,
...
async function ensureUsersExistForEmails(emails) {
    if (emails.length === 0)
        return;
...
export async function renameConversation(conversationId, actorUserId, newName) {
...
export async function getCohortMembersForMessaging(cohortId) {
    ...
    const courseTutors = cohort?.offering?.courseId
        ...
        : [];
    const tutorUserIds = new Set(courseTutors
        .map((assignment) => assignment.tutor.userId)
        .filter((value) => typeof value === "string" && value.length > 0));
    const membershipByUserId = new Map(memberships
    ...
    const learnerUserIds = Array.from(new Set(memberships
        .map((membership) => membership.userId)
        .filter((value) => typeof value === "string" && value.length > 0)));
    const learnerUsers = learnerUserIds.length > 0 || membershipEmails.length > 0
...
async function ensureTeamConversation(cohortId, batchNo) {
...
async function ensureBroadcastConversation(cohortId) {
    let conversation = await prisma.cpConversation.findFirst({
...
export async function createTeamConversationForCohort(cohortId, actorUserId, input) {
    ...
    const requestedMemberIds = Array.from(new Set(input.memberUserIds.map((value) => String(value).trim()).filter(Boolean)));
    ...
    if (nonStaffUsers.length > 0) {
        ...
        const validSet = new Set(validMemberships.flatMap((membership) => [
            membership.userId ?? "",
            membership.email.toLowerCase(),
        ]));
        const invalid = nonStaffUsers.find((user) => !validSet.has(user.userId) && !validSet.has(user.email.toLowerCase()));
        if (invalid) {
...
async function syncCohortConversationsForUser(cohortId, userId) {
...
export async function getConversationsForUser(userId, cohortId) {
    if (cohortId) {
...
export async function getConversationHistory(conversationId, userId, limit = 50, before) {
    await ensureConversationMembership(conversationId, userId);
    ...
    return messages.map((message) => {
        ...
        const isSeen = seenStates.some((s) => s.userId !== message.senderId &&
...
export async function getSeenIndicators(messageId) {
...
export async function getUnseenCounts(userId) {
    const memberships = await prisma.cpConversationMember.findMany({
        where: { userId },
...
export async function submitPollVote(messageId, userId, optionIndex, allowMultiple) {
...
export async function getMessagePollVotes(messageId) {
