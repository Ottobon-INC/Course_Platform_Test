...
export async function buildTutorCourseSnapshot(courseId) {
  ...
  `);
    ...
    progressRows.forEach((row) => {
        const entry = progressByUser.get(row.user_id) ?? { passedModules: new Set(), lastActivity: null };
        if (row.quiz_passed) {
    ...
    const now = new Date();
    const newThisWeek = learners.filter((learner) => differenceInDays(now, learner.enrolledAt) <= 7).length;
    ...
    const averageCompletion = learners.length === 0
        ? 0
...
export function formatTutorSnapshot(snapshot) {
