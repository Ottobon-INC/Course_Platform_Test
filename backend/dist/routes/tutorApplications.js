...
const applicationSchema = z.object({
    ...
    headline: z.string().min(8).max(240),
    courseTitle: z.string().min(4).max(200),
    courseDescription: z.string().min(24).max(4000),
    targetAudience: z.string().min(12).max(2000),
    expertiseArea: z.string().min(3).max(200),
    experienceYears: z.number().int().min(0).max(60).optional(),
    availability: z.string().min(3).max(200),
});
tutorApplicationsRouter.post("/", asyncHandler(async (req, res) => {
