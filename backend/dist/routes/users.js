...
usersRouter.patch("/profile", requireAuth, asyncHandler(async (req, res) => {
    const auth = req.auth;
    ...
    if (collegeName !== undefined)
        studentUpdate.collegeName = collegeName;
    if (branch !== undefined)
        studentUpdate.branch = branch;
    if (yearOfPassing !== undefined)
        studentUpdate.yearOfPassing = yearOfPassing;
    if (isCollegeStudent !== undefined)
        studentUpdate.isCollegeStudent = isCollegeStudent;
...
usersRouter.post("/profile/photo", requireAuth, upload.single("photo"), asyncHandler(async (req, res) => {
    const auth = req.auth;
    if (!req.file) {
...
usersRouter.get("/leaderboard/summary", requireAuth, asyncHandler(async (req, res) => {
    const auth = req.auth;
...
usersRouter.get("/leaderboard/top", requireAuth, asyncHandler(async (req, res) => {
    const auth = req.auth;
