...
messagingRouter.get("/attachments/:driveItemId/content", asyncHandler(async (req, res) => {
...
messagingRouter.get("/public/download", asyncHandler(async (req, res) => {
...
messagingRouter.post("/attachments/preview-url", asyncHandler(async (req, res) => {
    const auth = req.auth;
...
messagingRouter.get("/cohort-members", asyncHandler(async (req, res) => {
...
messagingRouter.post("/conversations/dm", asyncHandler(async (req, res) => {
    const auth = req.auth;
...
messagingRouter.post("/conversations/cohort/:cohortId/team", asyncHandler(async (req, res) => {
    const auth = req.auth;
    const cohortId = req.params.cohortId;
    const title = typeof req.body.title === "string" ? req.body.title.trim() : "";
    const memberUserIds = Array.isArray(req.body.memberUserIds)
        ? req.body.memberUserIds.filter((value) => typeof value === "string")
        : [];
    const requestedBatchNo = req.body.batchNo == null ? null : Number.parseInt(String(req.body.batchNo), 10);
    ...
    if (batchNo <= 0) {
        res.status(400).json({
            message: "Provide either (title + memberUserIds[]) for custom team chat or a valid positive batchNo",
        });
        return;
    }
...
messagingRouter.post("/conversations/cohort/:cohortId/broadcast", asyncHandler(async (req, res) => {
    const auth = req.auth;
    const cohortId = req.params.cohortId;
    const conversations = await messagingService.getConversationsForUser(auth.userId, cohortId);
...
messagingRouter.get("/conversations/:conversationId/history", asyncHandler(async (req, res) => {
    const auth = req.auth;
    const conversationId = req.params.conversationId;
    const limit = Number.parseInt(String(req.query.limit ?? 50), 10);
    const before = typeof req.query.before === "string" ? req.query.before : undefined;
    const messages = await messagingService.getConversationHistory(conversationId, auth.userId, Number.isFinite(limit) ? Math.max(1, Math.min(limit, 200)) : 50, before);
    res.status(200).json({ messages });
}));
messagingRouter.post("/messages", asyncHandler(async (req, res) => {
    const auth = req.auth;
...
messagingRouter.post("/messages/:messageId/read", asyncHandler(async (req, res) => {
    const auth = req.auth;
...
messagingRouter.get("/conversations/unseen-counts", asyncHandler(async (req, res) => {
    const auth = req.auth;
    const counts = await messagingService.getUnseenCounts(auth.userId);
    res.status(200).json({ counts });
}));
messagingRouter.get("/messages/:messageId/seen-by", asyncHandler(async (req, res) => {
    const seenBy = await messagingService.getSeenIndicators(req.params.messageId);
    res.status(200).json({ seenBy });
}));
messagingRouter.put("/conversations/:conversationId/rename", asyncHandler(async (req, res) => {
    const auth = req.auth;
    const conversationId = req.params.conversationId;
...
messagingRouter.post("/conversations/:conversationId/members", asyncHandler(async (req, res) => {
    const auth = req.auth;
    const conversationId = req.params.conversationId;
...
messagingRouter.post("/upload", (req, res, next) => {
