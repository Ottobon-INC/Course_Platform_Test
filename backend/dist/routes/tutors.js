...
async function isTutorForCourse(userId, courseId) {
...
tutorsRouter.post("/login", asyncHandler(async (req, res) => {
...
tutorsRouter.post("/assistant/query", requireAuth, requireTutor, asyncHandler(async (req, res) => {
    const auth = req.auth;
    if (!auth) {
...
tutorsRouter.get("/me/courses", requireAuth, requireTutor, asyncHandler(async (req, res) => {
    const auth = req.auth;
    if (!auth) {
        res.status(401).json({ message: "Unauthorized" });
        return;
...
tutorsRouter.get("/:courseId/enrollments", requireAuth, requireTutor, asyncHandler(async (req, res) => {
    const auth = req.auth;
...
tutorsRouter.get("/:courseId/progress", requireAuth, requireTutor, asyncHandler(async (req, res) => {
    const auth = req.auth;
    const { courseId } = req.params;
    if (!auth) {
        res.status(401).json({ message: "Unauthorized" });
    ...
    const progressRows = await prisma.$queryRaw(Prisma.sql `
    ...
    progressRows.forEach((row) => {
        ...
        const entry = progressByUser.get(row.user_id) ?? { passedModules: new Set() };
