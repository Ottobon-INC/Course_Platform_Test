...
lessonsRouter.get("/courses/:courseKey/topics", asyncHandler(async (req, res) => {
    ...
    if (uuidRegex.test(courseKey)) {
        ...
        const normalizedName = courseKey.replace(/-/g, " ");
...
lessonsRouter.get("/:lessonId/progress", asyncHandler(async (req, res) => {
...
lessonsRouter.put("/:lessonId/progress", asyncHandler(async (req, res) => {
    const { lessonId } = req.params;
    const userId = req.headers["x-user-id"] ?? "anonymous";
