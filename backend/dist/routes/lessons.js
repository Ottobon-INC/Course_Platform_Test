...
function normalizeVideoUrl(url) {
    ...
    try {
        ...
        const toEmbed = (id) => (id ? `https://www.youtube.com/embed/${id}` : trimmed);
...
lessonsRouter.get("/courses/:courseKey/progress", requireAuth, asyncHandler(async (req, res) => {
    const { courseKey } = req.params;
    const auth = req.auth;
    if (!auth?.userId) {
        res.status(401).json({ message: "Unauthorized" });
...
lessonsRouter.get("/:lessonId/progress", requireAuth, asyncHandler(async (req, res) => {
    ...
    const progressPercent = clampProgress(record?.lastPosition ?? 0);
    const status = record?.isCompleted ?? progressPercent >= 100
        ? "completed"
        : progressPercent > 0
            ? "in_progress"
            : "not_started";
...
lessonsRouter.put("/:lessonId/progress", requireAuth, asyncHandler(async (req, res) => {
    const { lessonId } = req.params;
    const auth = req.auth;
    if (!auth?.userId) {
        res.status(401).json({ message: "Unauthorized" });
    ...
    const normalizedStatus = record.isCompleted
        ? "completed"
