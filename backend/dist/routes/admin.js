...
adminRouter.get("/tutor-applications", requireAuth, requireAdmin, asyncHandler(async (_req, res) => {
    const applications = await prisma.tutorApplication.findMany({
        orderBy: { createdAt: "desc" },
    });
    res.status(200).json({ applications });
}));
adminRouter.post("/tutor-applications/:applicationId/approve", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
    ...
    const courseTitle = application.courseTitle?.trim() ||
    ...
    const courseDescription = application.courseDescription?.trim() || "Course description to be provided by tutor.";
