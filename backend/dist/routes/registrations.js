        ...
        const questions = await prisma.assessmentQuestion.findMany({
            where: {
                ...
                AND: [
                    ...
                    {
                        OR: [{ programType: "all" }, { programType: programType }],
                    },
                ],
            },
...
registrationsRouter.post("/", async (req, res, next) => {
    try {
        const { offeringId, fullName, email, phoneNumber, collegeName, yearOfPassing, branch, referredBy, selectedSlot, sessionTime, mode, status, answersJson, questionsSnapshot, assessmentSubmittedAt, plan, } = req.body ?? {};
