...
import { recordActivityEvents, getLatestStatusesForCourse, getLearnerHistory, ensureTutorOrAdminAccess, } from "../services/activityEventService";
...
activityRouter.post("/events", requireAuth, asyncHandler(async (req, res) => {
    const auth = req.auth;
    if (!auth) {
...
activityRouter.get("/courses/:courseId/learners", requireAuth, asyncHandler(async (req, res) => {
    const auth = req.auth;
    if (!auth) {
        res.status(401).json({ message: "Unauthorized" });
        return;
    ...
    const summary = learners.reduce((acc, learner) => {
        const key = (learner.derivedStatus ?? "unknown");
        if (key in acc) {
            acc[key] += 1;
        }
        else {
...
activityRouter.get("/learners/:learnerId/history", requireAuth, asyncHandler(async (req, res) => {
    const auth = req.auth;
    if (!auth) {
        res.status(401).json({ message: "Unauthorized" });
        return;
