...
import { getPersonaPromptTemplate, PERSONA_KEYS, type PersonaKey } from "./personaPromptTemplates";
...
export async function processUserQuery(params: {
    ...
    const normalizedPersonaKey =
      personaKey && PERSONA_KEYS.includes(personaKey as PersonaKey)
        ? (personaKey as PersonaKey)
        : null;
