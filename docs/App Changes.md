...
- **Pre-check enrollment** – CourseDetails now calls `POST /courses/:courseKey/enroll?checkOnly=true` to validate cohort access before opening the Ottolearn Protocol modal.
...
- Landing page CTAs, Auth callback, and nav links now route directly to `/course/ai-native-fullstack-developer/learn/welcome-to-ai-journey` instead of the retired dashboard/cart pages (legacy `/course/ai-in-web-development/learn/welcome-to-ai-journey` still resolves).
